...
public class Learner<P, S> {
	...
	public SRA<P, S> learn(Oracle<P, S> o, BooleanAlgebra<P, S> ba) throws TimeoutException {
		ObsTable table = new ObsTable(ba.generateWitness(ba.True()));
		
		SRA<P, S> conjecture = null;
		List<S> cx = null;
	...
	private class ObsTable {
        ...
        private List<List<S>> getAllSuffixes(List<S> w) {
            List<List<S>> suffixes = new ArrayList<List<S>>();
            System.out.println(w);
            List<S> suffix;
            for (int i = w.size() - 1; i >= 0; i--) {
                suffix = w.subList(i,w.size());
        ...
        public void process(List<S> cx) {
			List<List<S>> suffixes = getAllSuffixes(cx);

//                    prefixes = new ArrayList<List<S>>();
...
//			}
			...
			for (List<S> p : suffixes) {
                // Add to columns
                if (!E.contains(p)) {
                    E.add(p);

                    // NOT evidence-closure, the following is only for the additional columns
                    for(List<S> w : S) {
                        List<S> we = new ArrayList<>(w);
                        we.addAll(p);

                        // Add reachable strings to rows
                        if (!SUR.contains(we)) {
                            SUR.add(we);
                            R.add(we);
                        }
...
//			}
		...
		public boolean consistent(SRA<P, S> sra, BooleanAlgebra<P, S> ba) throws TimeoutException {
			for (List<S> w : SUR) {
				for (List<S> e : E) {
					List<S> we = new ArrayList<S>(w);
					we.addAll(e);
					if (!f.get(we).equals(sra.accepts(we, ba))) {
...
//		public SRA<P, S> buildSRA(BooleanAlgebra<P, S> ba) throws TimeoutException {
